...
Recent assessments of UK flood defence systems highlight the varied challenges faced by different regions. Studies [@MILLER2017345; @sayers2023present; @priestley2022flood] report that approximately 1.7 million properties in England are at significant risk of flooding, with coastal areas facing additional pressures from rising sea levels and storm surges. This risk is not evenly distributed, with some regions experiencing disproportionate challenges due to their geographical and climatic characteristics [@sayers2023present].
...
The simulation incorporated two types of variables:

Categorical Variables:

•	Region (reflecting the five primary regions in England)

•	Coastal status (Coastal/Inland, maintaining the original dataset's geographical distinctions)

•	Population size (Small/Medium/Large, based on local authority classifications)

Numerical Variables:

•	Annual maintenance budget (normally distributed, with parameters derived from typical local authority spending patterns)

•	Number of defences (Poisson distributed, reflecting the count-based nature of defence assets)

•	Defence condition ratings (1-5 scale, where 1 represents the best condition, matching the Environment Agency's rating system)

•	Annual flood incidents (modelled on historical patterns)

•	Proportion of defences above required condition (based on Environment Agency standards)

The relationships between variables were modelled to reflect real-world patterns observed in the NAO dataset:

•	Maintenance budgets were scaled according to population size, reflecting actual local authority resource allocation patterns

•	Defence counts were adjusted based on both population size and coastal status, mirroring the higher density of defences in coastal areas

•	Defence conditions were linked to maintenance budgets and regional factors, incorporating geographical variations in maintenance challenges

•	Flood incidents were modelled to reflect both defence condition and coastal proximity

Statistical Analysis: To examine the relationships between these variables, four primary analytical approaches were employed:
